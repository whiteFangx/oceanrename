...
class RenameManeuver(DefaultManeuver):
    ...
    async def execute(self) -> None:
        ...
        try:
            ...
            if self._fltr_obj.has_filters():
                ...
                await self._cmd_message.reply_text(
                    f"{Trans.RENAME_FILTER_MATCH_USED}\nFile name:- {new_file_name}"
                )
            else:
            ...
            [
                [
                    InlineKeyboardButton(
                        Trans.RENAME_CANCEL, f"cancel {self._unique_id}"
                    )
                ]
            ]
        )
        ...
        try:
            ...
            dl_path = os.path.join(f'downloads/{str(time.time()).replace(".", "")}/')
        ...
        if dl_path is None:
            renamelog.info("Download Canceled.")
        ...
        renamelog.debug(f"file size {str(os.path.getsize(dl_path))}")
        udb = UserDB()
        ...
        is_force = (
            mode_choice == udb.MODE_SAME_AS_SENT
            and self._media_message.document is not None
            or mode_choice != udb.MODE_SAME_AS_SENT
            and mode_choice == udb.MODE_AS_DOCUMENT
        )
        try:
